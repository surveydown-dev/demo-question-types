...
It also is an example of the conditional display logic. If you choose the "Other" option, a second question pops up below asking to specify which other type of penguin is your favorite. This is controlled with the `sd_show_if()` function in the `server` of `app.R`.
...
This page showcases the conditional skip logic, which allows you to skip forward in the survey based on different conditions. Depending on your choice, you'll go to a different page in the survey when you press the "Next" button. This is controlled with the `sd_skip_if()` function to the `server` of `app.R`.
